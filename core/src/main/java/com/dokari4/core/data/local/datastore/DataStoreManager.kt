package com.dokari4.core.data.local.datastore
...
class DataStoreManager(context: Context) {
    ...
    val checkOnboardingState: Flow<OnboardingState> = dataStore.data.map { preferences ->
        when (preferences[com.dokari4.core.data.local.datastore.DataStoreManager.Companion.ONBOARDING_KEY] ?: false) {
    ...
    suspend fun setOnboardingState(onboardingState: OnboardingState) {
        dataStore.edit { preferences ->
            preferences[com.dokari4.core.data.local.datastore.DataStoreManager.Companion.ONBOARDING_KEY] = when (onboardingState) {
