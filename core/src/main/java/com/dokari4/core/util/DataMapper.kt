...
object DataMapper {
    ...
    fun mapDomainToUserEntity(input: User) =
        UserEntity(id = input.id, name = input.name)
    ...
    fun mapAccountWithTransactionsEntityToDomain(input: List<AccountWithTransactions>):
            List<com.dokari4.core.domain.model.AccountWithTransactions> =
        input.map { entity ->
            com.dokari4.core.domain.model.AccountWithTransactions(
                id = entity.id,
                name = entity.name,
                accountType = entity.accountType,
                amount = entity.amount,
    ...
    fun mapCategoryWithTransactionsEntityToDomain(input: List<CategoryCountTotal>):
            List<com.dokari4.core.domain.model.CategoryCountTotal> =
        input.map { entity ->
            com.dokari4.core.domain.model.CategoryCountTotal(
                id = entity.id,
                name = entity.name,
                count = entity.count,
                amount = entity.amount
